...
function dataJSON() {
	return src(path.src.dataJSON)
		.pipe(dest(path.build.dataJSON));
}
...
function watchFiles() {
	...
	gulp.watch([path.watch.dataJSON], dataJSON);
}

const build = gulp.series(clean, gulp.parallel(html, css, js, images, fonts, dataJSON));
